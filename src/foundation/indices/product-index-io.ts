...
import { listJsonDir, readJson } from "@/src/foundation/data";
import {
...
export async function validateProductIndexConsistency(
  ...
  indexTotal: number;
  fileCount: number;
  deep: import("@/src/foundation/indices/product-index-consistency").IndexConsistencyReport;
}> {
  const { auditProductIndexConsistency } = await import(
    "@/src/foundation/indices/product-index-consistency"
  );
  const current =
