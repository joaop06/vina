...
import { readEntityDir } from "@/src/foundation/indices/read-entity-dir";
import {
...
} from "@/src/foundation/indices/paths";
import {
...
export async function validateClientIndexConsistency(
  ...
  indexTotal: number;
  fileCount: number;
  deep: import("@/src/foundation/indices/client-index-consistency").ClientIndexConsistencyReport;
}> {
  const { auditClientIndexConsistency } = await import(
    "@/src/foundation/indices/client-index-consistency"
  );
